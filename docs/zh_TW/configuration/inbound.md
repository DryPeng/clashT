...
任何入站协议的连接都将由同一个内部规则匹配引擎处理. 也就是说, ClashTT **目前**不支持为不同的入站协议设置不同的规则集.
...
Redirect 和 TProxy 是两种实现透明代理的不同方式, 均被 ClashTT 所支持.

然而, 您不一定需要手动设置这两个功能 - 我们建议您使用 [ClashTT Premium 版本](/zh_CN/premium/introduction) 来配置透明代理, 因为它内置了对操作系统路由表、规则和 nftables 的自动管理.
