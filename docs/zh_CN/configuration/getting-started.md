...
建议您在继续阅读本节之前, 先阅读[介绍](/zh_CN/configuration/introduction). 在您对ClashTT的工作原理有了简单的了解后, 您可以开始编写您自己的配置.

## 配置文件

主配置文件名为 `config.yaml`. 默认情况下, ClashTT会在 `$HOME/.config/clash` 目录读取配置文件. 如果该目录不存在, ClashTT会在该位置生成一个最小的配置文件.
...
在某些情况下, 您需要匹配通配符域名. 例如, 当您设置 [ClashTT DNS](/zh_CN/configuration/dns) 时, 您可能想要匹配 `localdomain` 的所有子域名.

ClashTT 在 DNS 配置中提供了匹配不同级别通配符域名的支持, 其语法如下:
